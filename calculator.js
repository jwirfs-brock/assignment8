...
// Start in Single Button mode
$(document).ready(function() {
	autoNext(speed);
	$("#single-button").addClass("active");
  $("#three-button").removeClass("active");
	$("#single-inst").show();
	$("#three-inst").hide();
});

$("#single-button").click(function() { // when the "Launch Single" mode button is clicked
  autoNext(speed);
	$("#single-button").addClass("active");
	$("#three-button").removeClass("active");
	$("#single-inst").show();
...
$("#three-button").click(function() { // when the "Launch Three Button" mode button is clicked
  autoStop(speed);
	$("#single-button").removeClass("active");
	$("#three-button").addClass("active");
	$("#single-inst").hide();
	$("#three-inst").show();
});
...
// This function progresses the buttons on a timer (adapted from http://mdn.beonex.com/en/DOM/window.setInterval.html)
var speed = 500;
var intervalID;
function autoNext(speed){
	intervalID = setInterval(selectNext, speed);
}

// This function stops the buttons so they no longer progress on a timer
function autoStop(){
